...
public class ResponseBodyController {
    ...
    /**
     * ResponseEntity 는 HttpEntity 를 상속받았는데
     * HttpEntity 는 HTTP 메시지의 헤더,바디 정보를 가지고 있다.
     * ResponseEntity 는 여기에 더해서 HTTP 응답코드를 설정할 수 있다.
     */
    ...
    public HttpEntity<String> responseBodyV2_1() {
        return new HttpEntity<>("v2-1 ok 번외로 만들어 보았다.");
    }

    /**
     * @ResponseBody 를 사용하면 view 를 사용하지 않고
     ...
     * HTTP 메시지 컨버터를 통해서 JSON 형식으로 변환되어서 반환한다.
     */
    @GetMapping("/response-body-json-v1")
    public ResponseEntity<HelloData> responseJsonV1() {
        HelloData helloData = new HelloData();
        helloData.setUsername("김이름");
        helloData.setAge(20);
    ...
    /**
     * RespsoneEntity 는 HTTP 응답코드를 설정할 수 있는데
     * @ResponseBody 를 사용하면 이런 것을 설정하기 까다롭다.
     * @ResponseStatus(응답코드) 애노테이션을 사용하면 응답 코드도 설정할 수 있다.
     * 물론 애노테이션이기 때문에 응답코드를 동적으로 변경할 수는 없다.
     * 프로그램 조건에 따라서 동적으로 변경하려면
